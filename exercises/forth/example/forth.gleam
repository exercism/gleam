...
fn toke_word(w: String, t: List(String)) -> tuple(ForthTok, List(String)) {
  case w {
    ":" -> {
      let tuple([w, ..def], [_, ..rest]) = list.split_while(t, fn(c) { case string.compare(c, ";") {
        order.Eq -> False
        _ -> True
      }});
...
fn binary_op(f: Forth, op: fn(Int, Int) -> Result(Int, ForthError)) -> Result(Forth, ForthError) {
  try tuple(a, rest) = stack_pop(f.stack);
  try tuple(b, rest0) = stack_pop(rest);
  try i = op(b,a); // order is important
...
fn execute_builtin(f: Forth, builtin: String) -> Result(Forth, ForthError) {
  case builtin {
    ...
    "DUP" -> {
      ...
      Ok(Forth(..f, stack: stack_push(rest0, h) |> stack_push(h)))
    }
    "DROP" -> {
    ...
    "SWAP" -> {
      ...
      Ok(Forth(..f, stack: stack_push(rest1, a) |> stack_push(b)))
    }
    "OVER" -> {
      try tuple(a, rest0) = stack_pop(f.stack);
      try tuple(b, rest1) = stack_pop(rest0);
      Ok(Forth(..f, stack: stack_push(rest1, b) |> stack_push(a) |> stack_push(b)))
    }
  }
}
...
fn eval_token(f: Forth, tok: ForthTok) -> Result(Forth, ForthError) {
  case tok {
    ...
    Word(w) -> { // load word and evaluate with stack
    ...
    WordDef(w, def) -> { // define new word
      case int.parse(w) { // room for improvement here, but this is sufficent for our needs.
