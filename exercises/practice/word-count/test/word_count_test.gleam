...
pub fn count_one_word_test() {
  ...
  |> should.equal(map.from_list([#("word", 1)]))
}

pub fn count_one_of_each_word_test() {
  "one of each"
  |> word_count.count_words
  |> should.equal(map.from_list([#("one", 1), #("of", 1), #("each", 1)]))
}
...
pub fn handles_cramped_lists_test() {
  "one,two,three"
  |> word_count.count_words
  |> should.equal(map.from_list([#("one", 1), #("two", 1), #("three", 1)]))
}
...
pub fn include_numbers_test() {
  "testing, 1, 2 testing"
  |> word_count.count_words
  |> should.equal(map.from_list([#("testing", 2), #("1", 1), #("2", 1)]))
}

pub fn normalize_case_test() {
  "go Go GO Stop stop"
  |> word_count.count_words
  |> should.equal(map.from_list([#("go", 3), #("stop", 2)]))
}
...
pub fn multiple_spaces_not_detected_as_a_word_test() {
  ...
  |> word_count.count_words
  |> should.equal(map.from_list([#("multiple", 1), #("whitespaces", 1)]))
}
...
pub fn quotation_for_word_with_apostrophe_test() {
  "can, can't, 'can't'"
  |> word_count.count_words
  |> should.equal(map.from_list([#("can", 1), #("can't", 2)]))
}
