...
pub fn unicode_letters_test() {
  parallel_letter_frequency.calculate_frequencies(["本", "φ", "ほ", "ø"])
  |> should.equal(
    dict.from_list([#("本", 1), #("φ", 1), #("ほ", 1), #("ø", 1)]),
  )
}

pub fn combination_of_lower_and_uppercase_letters_punctuation_and_white_space_test() {
  parallel_letter_frequency.calculate_frequencies([
...
pub fn many_small_texts_test() {
  ...
  |> should.equal(dict.from_list([#("a", 50), #("b", 100), #("c", 150)]))
}
