...
pub fn each_item_may_only_be_read_once_test() {
  let buffer = circular_buffer.new(1)
  assert Ok(buffer) = circular_buffer.write(buffer, 1)
  assert Ok(#(1, buffer)) = circular_buffer.read(buffer)
  assert Error(Nil) = circular_buffer.read(buffer)
}
...
pub fn full_buffer_cant_be_written_to_test() {
  let buffer = circular_buffer.new(1)
  assert Ok(buffer) = circular_buffer.write(buffer, 1)
  assert Error(Nil) = circular_buffer.write(buffer, 2)
}
...
pub fn read_position_is_maintained_even_across_multiple_writes_test() {
  ...
  assert Ok(buffer) = circular_buffer.write(buffer, 2)
  assert Ok(#(1, buffer)) = circular_buffer.read(buffer)
  assert Ok(buffer) = circular_buffer.write(buffer, 3)
  assert Ok(#(2, buffer)) = circular_buffer.read(buffer)
  assert Ok(#(3, _)) = circular_buffer.read(buffer)
}
...
pub fn overwrite_replaces_the_oldest_item_remaining_in_buffer_following_a_read_test() {
  ...
  assert Ok(buffer) = circular_buffer.write(buffer, 3)
  assert Ok(#(1, buffer)) = circular_buffer.read(buffer)
  assert Ok(buffer) = circular_buffer.write(buffer, 4)
  let buffer = circular_buffer.overwrite(buffer, 5)
  assert Ok(#(3, buffer)) = circular_buffer.read(buffer)
  assert Ok(#(4, buffer)) = circular_buffer.read(buffer)
  assert Ok(#(5, _)) = circular_buffer.read(buffer)
}
...
pub fn initial_clear_does_not_affect_wrapping_around_test() {
  ...
  assert Ok(#(3, buffer)) = circular_buffer.read(buffer)
  assert Ok(#(4, buffer)) = circular_buffer.read(buffer)
  assert Error(_) = circular_buffer.read(buffer)
}
