...
-opaque subject(ESH) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, ESH}.
...
-type call_error(ESJ) :: {callee_down, gleam@dynamic:dynamic()} |
    call_timeout |
    {gleam_phantom, ESJ}.
...
-spec 'receive'(subject(ESU), integer()) -> {ok, ESU} | {error, nil}.
...
-spec select(selector(ETA), integer()) -> {ok, ETA} | {error, nil}.
select(Field@0, Field@1) ->
    gleam_erlang_ffi:select(Field@0, Field@1).

-spec select_forever(selector(ETE)) -> ETE.
select_forever(Field@0) ->
    gleam_erlang_ffi:select(Field@0).

-spec map_selector(selector(ETI), fun((ETI) -> ETG)) -> selector(ETG).
map_selector(Field@0, Field@1) ->
    gleam_erlang_ffi:map_selector(Field@0, Field@1).

-spec merge_selector(selector(ETK), selector(ETK)) -> selector(ETK).
merge_selector(Field@0, Field@1) ->
    gleam_erlang_ffi:merge_selector(Field@0, Field@1).

-spec selecting_trapped_exits(selector(ETO), fun((exit_message()) -> ETO)) -> selector(ETO).
...
-spec selecting(selector(ETR), subject(ETT), fun((ETT) -> ETR)) -> selector(ETR).
...
-spec selecting_record3(
    selector(EUA),
    any(),
    fun((gleam@dynamic:dynamic(), gleam@dynamic:dynamic()) -> EUA)
) -> selector(EUA).
...
-spec selecting_record4(
    selector(EUE),
    any(),
    fun((gleam@dynamic:dynamic(), gleam@dynamic:dynamic(), gleam@dynamic:dynamic()) -> EUE)
) -> selector(EUE).
...
-spec selecting_anything(selector(EUI), fun((gleam@dynamic:dynamic()) -> EUI)) -> selector(EUI).
...
-spec try_call(subject(EUT), fun((subject(EUV)) -> EUT), integer()) -> {ok, EUV} |
    {error, call_error(EUV)}.
...
-spec call(subject(EVA), fun((subject(EVC)) -> EVA), integer()) -> EVC.
...
-spec send_after(subject(EVF), integer(), EVF) -> timer().
