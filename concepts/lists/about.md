...
Adding and removing elements from the start of a list is very fast, but adding and removing elements from the end of a list is slow. This is because lists are implemented as a singly linked list, so adding an element to the end of a list requires traversing the entire list to find the end.
